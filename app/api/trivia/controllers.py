...
def get_most_similar_question_controller(test_season, player):
    ...
    _, test_season_stats_df = get_player_index(get_season_stats_as_df(test_season), player['name'])

    for season in player['seasons']:

        player_row, season_stats_df = get_player_index(get_season_stats_as_df(season), player['name'])
